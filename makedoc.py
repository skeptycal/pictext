...
Config = type(
    ...
    (object,),
    {
        "TARGET_MODULE": importlib.import_module("pictext"),
        "OUTPUT_FILE": "README.md",
        "IGNORED_MEMBERS": ["VERSION"],
        "DEPOT_URL": "https://github.com/skeptycal/pictext",
    },
)
...
def build_member_doc(mi, is_sub=False):
    ...
    if mi.is_str or mi.is_number or mi.is_enum_val:
        doc = "> = {0}".format(
            '"{0}"'.format(
                str(mi.obj)) if mi.is_str or mi.is_enum_val else str(mi.obj)
        )
    else:
    ...
    info = info_tpl.format(
        link=mi.link, name=formatted_name, sign=sign, doc=doc)
